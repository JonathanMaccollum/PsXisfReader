...
while($true){
    ...
    Get-ChildItem $DropoffLocation *.xisf -ErrorAction Continue |
        ...
        #where-object Object -eq "Serpens on HD141827" |
        #where-object Filter -ne "Oiii3nm" |
            ...
            $lights |
                ...
                foreach-object {
                    ...
                    if($filter -in @('L','R','G','B')){
                        $masterFlat ="E:\Astrophotography\$($focalLength)mm\Flats\20220710.MasterFlatCal.$($filter)_cc.xisf"
                    }
                    else{
                        $masterFlat ="E:\Astrophotography\$($focalLength)mm\Flats\20220711.MasterFlatCal.$($filter).xisf"
                    }
