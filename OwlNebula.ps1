...
$target="E:\Astrophotography\1000mm\Owl Nebula"
...
$stats = get-childitem $target *.data.csv |
        ...
        foreach-object {
            ...
            $w=$null
            $pathToWeightedFile=join-path $WeightedOutputPath ($y.Path.Name.TrimEnd(".xisf")+"_a.xisf")
            if(($x.Approved -eq "true") -and (test-path $pathToWeightedFile)) {
                $w = Get-Item $pathToWeightedFile
            }
...
$referenceFrame = $stats|where-object ObsDate -eq "'2020-04-04T06:18:27.130'"

$stats | where-object Approved -eq "true" | where-object Filter -ne "'Sii'" |group-object Filter | foreach-object {
    $filter = $_.Group[0].Filter
...
$aligned = Get-XisfFile -Path $AlignedOutputPath |
    where-object object -eq "'Owl Nebula'" 
